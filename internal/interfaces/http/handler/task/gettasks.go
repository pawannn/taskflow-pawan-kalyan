...
func (h *taskHandler) getByProject(w http.ResponseWriter, r *http.Request) {
	...
	tasks, hasNext, err := h.taskService.GetByProjectID(ctx, projectID, taskStatus, assigneePtr, meta.UserID, limit, offset)
	if err != apperr.NoErr {
		if err.Data != nil {
			h.engine.Log.Error(ctx, "Fetch Task failed", "error", err.Data)
		}
	...
	h.engine.SendResponse(w, meta.ReqID, http.StatusOK, "tasks fetched", response)
}
